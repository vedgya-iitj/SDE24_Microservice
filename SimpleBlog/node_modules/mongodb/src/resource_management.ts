...
export function configureExplicitResourceManagement() {
  ...
  const { MongoClient } = require('mongodb/src/mongo_client');
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { ClientSession } = require('mongodb/src/sessions');
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { AbstractCursor } = require('mongodb/src/cursor/abstract_cursor');
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { ChangeStream } = require('mongodb/src/change_stream');
